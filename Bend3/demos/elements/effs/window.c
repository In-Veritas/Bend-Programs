...
//                                    since the last frame (ASCII), or
//                                    256 + 4*(local_y*300+local_x) + panel
//                                    for a panel click (earth, water, fire,
//                                    air = 0..3); 0 means none
//                                    and 'q' means the window was closed
...
//   2 x y+1000 scale glyph rgb   3x5 glyph: 0..9 digits, 10 B, 11 E,
//                                12 N, 13 D, 14 '!'
//
...
typedef CGPoint (*msg_point)(id, SEL);
...
// 3x5 font, one u16 per glyph, rows top-down, 3 bits per row
static unsigned short win_glyph(u32 d) {
  switch (d) {
    ...
    case 9: return 075717;  // 111 101 111 001 111
    case 10: return 065656; // 110 101 110 101 110  B
    case 11: return 074647; // 111 100 110 100 111  E
    case 12: return 075555; // 111 101 101 101 101  N
    case 13: return 065556; // 110 101 101 101 110  D
    default: return 022202; // 010 010 010 000 010  !
  }
}

static void win_digit(long x, long y, long s, u32 d, u32 rgb) {
  unsigned short g = win_glyph(d);
  for (long r = 0; r < 5; r++) {
    u32 row = (g >> ((4 - r) * 3)) & 7;
    for (long c = 0; c < 3; c++) {
      if (row & (1u << (2 - c))) {
...
static u32 point_click(CGPoint point) {
  long x = (long)point.x;
  long y = (long)win_h - 1 - (long)point.y;
  bool left = x >= 12 && x < 312;
  bool right = x >= 328 && x < 628;
  u32 panel = 0;
  long px = 0;
  long py = 0;
  if (y >= 76 && y < 228) {
  ...
  u32 local_x = (u32)(x - px);
  u32 local_y = (u32)(y - py);
  return 256 + 4 * (local_y * 300 + local_x) + panel;
}
  ...
  for (;;) {
    ...
    if (type == 10 /* keyDown */) {
      ...
      if (chars != NULL) {
        ...
        if (c != NULL && c[0] != 0) {
          win_event_seen = (u32)(unsigned char)c[0];
        }
      }
    } else if (type == 1 /* leftMouseDown */) {
      u32 click = point_click(((msg_point)objc_msgSend)(e, win_sel("locationInWindow")));
      if (click != 0) {
        win_event_seen = click;
      }
